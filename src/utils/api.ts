...
async function request(url: string, data: any) {
    ...
    if (response.code != 0) {
        console.log(`二级文档列表插件接口异常 url : ${url} , msg : ${response.msg}`)
    }
    return res;
}
...
export async function getNotebookMapByApi(): Promise<Map<string, INotebook>> {
    let notebooks: INotebook[] = (await lsNotebooks()).notebooks;
...
export async function createNotebook(name: string): Promise<INotebook> {
...
export async function listDocsByPath(
    ...
    maxListCount: number,
    showHidden: boolean,
): Promise<{ files: IFile[], box: string, path: string }> {
    /**
     * {
  "notebook": "20220902115243-k6ogmmk",
  "path": "/20220902221817-v7xndwe.sy",
  ...
  "showHidden": false
    ...
    let data = {
        ...
        path: path,
        sort: sort,
        ignoreMaxListHint: ignoreMaxListHint,
...
// **************************************** Block ****************************************
export async function getDocInfo(id: BlockId): Promise<any> {
    let data = {
        id: id
    }
